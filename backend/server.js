...
const MONGODB_URI =
  process.env.MONGODB_URI || 'mongodb://localhost:27017/healthcare_privacy';
...
app.post('/api/encrypt', async (req, res) => {
  try {
    const { patient_id, age, gender, disease, blood_pressure, risk_score } =
      req.body;
    ...
    const encrypted = cryptoResponse.data.encrypted_data;
    ...
    const record = new EncryptedPatientData({
      ...
      encrypted_blood_pressure: encrypted.blood_pressure,

      encrypted_risk_score: encrypted.risk_score,
    ...
    await AuditLog.create({
      ...
      details: `Encrypted patient ${patient_id}`,
          ...
          try {
            ...
            for (const row of rows) {
              try {
                ...
                if (!row.patient_id || !row.age || !row.disease) {
                  failed++;
                ...
                const record = new EncryptedPatientData({
                  ...
                  disease_category: row.disease,

                  upload_source: 'batch_csv',
                ...
                await AuditLog.create({
                  ...
                  details: `Uploaded patient ${row.patient_id}`,
...
app.get('/api/records', async (req, res) => {
  try {
    const records = await EncryptedPatientData.find().sort({ timestamp: -1 });
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    const totalComputations = await ComputationResult.countDocuments();

    const today = new Date();

    today.setHours(0, 0, 0, 0);

    const todayEncryptions = await EncryptedPatientData.countDocuments({
      timestamp: {
        $gte: today,
      },
    });
  ...
  async (req, res) => {
    try {
      ...
      records.forEach((record) => {
        ...
        const fixedIndex = dayIndex === 0 ? 6 : dayIndex - 1;

        encryptionData[fixedIndex].records++;
      });
      ...
      records.forEach((record) => {
        const disease = record.disease_category || 'Unknown';

        diseaseMap[disease] = (diseaseMap[disease] || 0) + 1;
      });

      const diseaseDistribution = Object.keys(diseaseMap).map((key) => ({
        name: key,
      ...
      const recentActivity = records
        ...
        .map((record) => ({
          ...
          func: 'Patient Record Encryption',
...
app.post('/api/compute', async (req, res) => {
  try {
    ...
    await AuditLog.create({
      ...
      details: `Executed ${function_type}`,
...
app.get('/api/results', async (req, res) => {
  try {
    const results = await ComputationResult.find().sort({
      generated_time: -1,
    });
  ...
  async (req, res) => {
    try {
      ...
      await AuditLog.create({
        ...
        details: `Started MPC session ${session_id}`,
